...
CREATE TABLE [NDTUAT].[tsuActiveTest] (
...
CREATE TABLE [NDTUAT].[tsuFailures] (
	[testResultID] [int] NOT NULL ,
	[test] [nvarchar] (255) NOT NULL ,
	[message] [nvarchar] (255) NOT NULL 
) ON [PRIMARY]
...
CREATE PROCEDURE [NDTUAT].tsu_describe
...
CREATE   PROCEDURE [NDTUAT].tsu__private_addFailure @test NVARCHAR(255), @errorMessage NVARCHAR(255)
-- GENERAL INFO:    This stored procedure is a part of the tsqlunit
--                  unit testing framework. It is open source software
--                  available at http://tsqlunit.sourceforge.net
--
...
CREATE PROCEDURE [NDTUAT].tsu__private_createTestResult @suiteName NVARCHAR(255)=''
-- GENERAL INFO:    This stored procedure is a part of the tsqlunit
--                  unit testing framework. It is open source software
--                  available at http://tsqlunit.sourceforge.net
--
...
CREATE  PROCEDURE NDTUAT.tsu__private_showTestResult 
...
CREATE PROCEDURE [NDTUAT].tsu_error 
	@errorNr INT 
-- GENERAL INFO:    This stored procedure is a part of the tsqlunit
--                  unit testing framework. It is open source software
--                  available at http://tsqlunit.sourceforge.net
...
CREATE  PROCEDURE NDTUAT.tsu_failure 
...
CREATE PROCEDURE [NDTUAT].tsu_runTestSuite @suite NVARCHAR(255)
-- GENERAL INFO:    This stored procedure is a part of the tsqlunit
--                  unit testing framework. It is open source software
--                  available at http://tsqlunit.sourceforge.net
--
...
INSERT INTO #result EXECUTE NDTUAT.tsu_describe
IF @@ERROR<>0 
BEGIN
	RETURN 100
END
...
EXEC @retVal=NDTUAT.tsu__private_createTestResult @suite
...
BEGIN
	...
	BEGIN
		...
		SET @spName= 'NDTUAT.' + @testPrefix +  @suite + '_setup'
		EXEC  @spName
		SET @setupError=@@ERROR
		IF (@setupError <> 0) 
			EXEC NDTUAT.tsu_error @setupError
	END
	IF @setupError= 0
	BEGIN
		...
		IF (@procedureError <> 0 AND @setupError=0 AND @failure=0)  -- Only show the first error 
			EXEC NDTUAT.tsu_error @procedureError
	END
	...
	BEGIN
		...
		SET @spName= 'NDTUAT.' +@testPrefix +  @suite + '_teardown'
		EXEC  @spName
		SET @tearDownError=@@ERROR
		IF (@tearDownError <> 0 AND @setupError = 0 AND @failure=0 AND @procedureError = 0 )
			EXEC NDTUAT.tsu_error @tearDownError  -- Only show the first error 
	END
	...
	IF @isError=1
	    EXEC [NDTUAT].tsu__private_addError @procedureName, @message
	ELSE IF @isFailure=1
	    EXEC [NDTUAT].tsu__private_addFailure @procedureName, @message
...
CREATE    PROCEDURE NDTUAT.tsu_showTestResults 
	@startTime datetime=NULL,
...
BEGIN
	...
	WHILE @@FETCH_STATUS =0 
	BEGIN
		EXEC NDTUAT.tsu__private_showTestResult @testResultID
		FETCH NEXT FROM cursortestResultID INTO @testResultID
	END
...
CREATE PROCEDURE [NDTUAT].tsu_runTests
...
INSERT INTO #tests EXECUTE NDTUAT.tsu_describe
...
WHILE @@FETCH_STATUS = 0
BEGIN
	EXEC NDTUAT.tsu_runTestSuite @testsuite	
...
EXEC [NDTUAT].tsu_showTestResults @startTime, @stopTime
...
ALTER  PROCEDURE [NDTUAT].[tsu_Assert]   
