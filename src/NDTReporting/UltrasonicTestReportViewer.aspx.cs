...
{
    public partial class UltrasonicTestReportViewer : System.Web.UI.Page
    {
        ...
        protected void btnExport_Click(object sender, EventArgs e)
        {
            ...
            DbUltrasonicTest obj = new DbUltrasonicTest();
            UltrasonicTestTemplate objUltrasonicTestTemplate = new UltrasonicTestTemplate();

            objUltrasonicTestTemplate = obj.GetUltrasonicTestReportById(ReportId);

            string paramBuild = "id=" + objUltrasonicTestTemplate.UltrasonicTest.ReportId +
                "&location=" + objUltrasonicTestTemplate.UltrasonicTest.Location +
            "&client=" + objUltrasonicTestTemplate.UltrasonicTest.FabricatorName +
            "&contractno=" + objUltrasonicTestTemplate.UltrasonicTest.ContractNo +
            "&reportno=" + objUltrasonicTestTemplate.UltrasonicTest.ReportNo +
            //"&description=" + objUltrasonicTestTemplate.UltrasonicTest.d +
            "&testdate=" + string.Format("{0:dd/MM/yyyy}", objUltrasonicTestTemplate.UltrasonicTest.DateofTesting) +
            "&contactperson=" + objUltrasonicTestTemplate.UltrasonicTest.ContactPerson;
        ...
        private bool DownloadableProduct_Tracking(string pdfFileName)
        {
            ...
            if (fileInfo.Exists)
            {
                ...
                {
                    long startBytes = 0;
                    //string lastUpdateTiemStamp = File.GetLastWriteTimeUtc().ToString("r");
        ...
        protected void btnAuthorize_Click(object sender, EventArgs e)
        {
            HandleError.HandleInternalError(() =>
            {
                ...
                if (ResultId > 0)
                {
                    Response.Redirect("UltrasonicTestReportList.aspx?SubTestId=3");
                }
            },
            (m, ex) => { });
        }
