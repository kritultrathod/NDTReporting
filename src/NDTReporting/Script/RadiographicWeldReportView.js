//Custom Declarations
{
    ...
    var handlerRep = "RadiographicWeldList.ashx";
    var ReportID = 0;
    var curntPage;
    var pageCount;
    var sortField = '5';
...
//Page Functions
{
        ...
        $(".PrintReport").live('click', function (e) {
            ...
            if (printAction == "Authorize") {
                if (ssnIsAdmin == 1) {
                    ...
                    DoAjaxCall(handler, "FillComboTemplates", "GetTemplateNames", "", datatype, JSON.stringify(req));
                    e.preventDefault();
                }
            }
    ...
    function GetAllRadiographicWeldReport() {
        ...
        var req = new SearchTestRecordReq(
            ...
            sortField,
            sortDir,
            $('#txtSrcTestReportNumber').val() == undefined ? '' : $('#txtSrcTestReportNumber').val(),
        ...
        DoAjaxCall(handler, "GetAllRadiographicWeldReportsCompleted", "GetAllRadiographicWeldReports", "", datatype, JSON.stringify(req));
    ...
    function GetAllRadiographicWeldReportsCompleted(data, message) {
        var str = "<table class='TableValue'>" +
            "<tr> <td colspan='10' class='TDHeader'> <b>Radiographic Test Report List</b> </td> </tr>";
        //Search Controls Row
        str += "<tr>" +
        ...
        str += "<tr> " +
            ...
            "<td class='TDHeaderCell1'>Contract No</td>" +
        ...
        for (var i = 0; i < data.TestRecordLightList.length; i++) {
            ...
            str += "<td class='TDFieldCell1'><a href='javascript:void(0)' class='classname " + GetLinkState() + " id='" + data.TestRecordLightList[i].ReportId + "' " + GetDisableButtonState(data.TestRecordLightList[i].IsAuthorized) + " >" + GetPrintLabel(data.TestRecordLightList[i].IsAuthorized) + "</a></td>";
            str += "</tr>";
        }
        str += "<tr>";
    ...
    function DeleteReport(ReportID) {
        if (confirm("Are you sure you want to delete the record?")) {
            DoAjaxCall(handlerRep, "DeleteRadiographicWeldReportSucess", "deleteRadiographicWeldReport", ReportID, datatype, empty);
        }
    }
    function EditReport(ReportID) {
        
        window.location.href = "RadiographicWeldReport.aspx?ReportID=" + ReportID;
    }
    function DeleteRadiographicWeldReportSucess(data, message) {
        GetAllRadiographicWeldReport();
        alert(message);
    }
    function Print() {
        ...
        DoAjaxCall(handler, "RedirectToReportViewParams", "SetNavParams", "", datatype, JSON.stringify(req));
        HideDialog();
    }
    function RedirectToReportViewParams(data, message) {
        window.location.href = "RadiographicWeldReportViewer.aspx";
    }
