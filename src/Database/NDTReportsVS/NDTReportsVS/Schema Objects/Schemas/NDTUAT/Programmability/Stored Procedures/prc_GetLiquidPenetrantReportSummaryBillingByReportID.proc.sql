CREATE PROCEDURE [NDTUAT].[prc_GetLiquidPenetrantReportSummaryBillingByReportID]        
 @ClientID AS INT,        
 @ReportId AS INT,    
...
SELECT @SQL = N'    
  ...
  SELECT @TotalQuantumOfJobSquareInch = SUM(    
   CONVERT(Decimal(18,2),RWTR.LiquidQuantityX) *    
   CONVERT(Decimal(18,2),RWTR.LiquidQuantityY)),    
  ...
  FROM  NDTUAT.LiquidPenetrantExaminationTestResults RWTR     
 LEFT OUTER JOIN     
  NDTUAT.LiquidPenetrantExaminationTest RW     
