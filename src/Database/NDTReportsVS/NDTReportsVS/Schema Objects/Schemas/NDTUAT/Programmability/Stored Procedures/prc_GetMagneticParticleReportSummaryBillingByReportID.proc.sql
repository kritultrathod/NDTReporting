CREATE PROCEDURE [NDTUAT].[prc_GetMagneticParticleReportSummaryBillingByReportID]        
...
SELECT @SQL = N'    
  ...
  SELECT @TotalQuantumOfJobSquareInch = SUM(    
   CONVERT(Decimal(18,2),RWTR.MagneticDimensionX) *    
   CONVERT(Decimal(18,2),RWTR.MagneticDimensionY)),    
  ...
  FROM NDTUAT.MagneticParticleExaminationTestResults RWTR     
 LEFT OUTER JOIN     
 NDTUAT.MagneticParticleExaminationTest RW     
