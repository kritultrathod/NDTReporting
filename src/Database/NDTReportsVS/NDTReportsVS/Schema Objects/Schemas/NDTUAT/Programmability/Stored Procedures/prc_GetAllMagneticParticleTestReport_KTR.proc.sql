CREATE PROCEDURE [NDTUAT].[prc_GetAllMagneticParticleTestReport_KTR]
	@CreatedBy as int,
    @pageIndex int = 1,  
    @PageSize int = 10,
...
SELECT
	...
	/*		1	Test Report No.
			...
			6	Location*/
	CASE
		WHEN @OrderBy = 1 AND @OrderDir = 'A' THEN T.DateofTesting
	END ASC,
	CASE
		WHEN @OrderBy = 1 AND @OrderDir = 'D' THEN T.DateofTesting
	END DESC,
	CASE
	...
	END DESC
	) AS RowId INTO #MagneticParticleExaminationTest
FROM NDTUAT.MagneticParticleExaminationTest T
...
SELECT
	@MyRowCount = COUNT(1)
FROM #MagneticParticleExaminationTest
