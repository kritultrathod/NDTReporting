CREATE PROCEDURE [NDTUAT].[prc_GetRadiographicReportSummaryBillingByReportID]      
 @ClientID AS INT,      
 @ReportId AS INT,  
...
SELECT @SQL = N'  
  ...
  SELECT @TotalQuantumOfJobSquareInch = SUM(  
   CONVERT(Decimal(18,2),RWTR.FlimSizeX) *  
   CONVERT(Decimal(18,2),RWTR.FlimSizeY)),  
  ...
  @PerUnitRate = FTR.PerUnitRate,  
  @PerDayUnitRate = PDR.PerDayUnitRate  
  FROM NDTUAT.RADIOGRAPHICWELDTESTRESULT RWTR   
 LEFT OUTER JOIN   
 NDTUAT.RADIOGRAPHICWELD RW   
  ON RW.ReportId = RWTR.ReportId    
 LEFT OUTER JOIN   
 NDTUAT.FilmTypeRates FTR  
  ON FTR.SubTest = RW.SubTestID    
  AND FTR.FilmType = RW.FilmMakeAndType  
  AND FTR.ClientID = @ClientIDParameter   
 LEFT OUTER JOIN   
 NDTUAT.PerDayRates PDR    
  ON PDR.SubTestid = RW.SubTestID  
  AND PDR.RadiationSourceID = RW.RadiationSourceID  
